...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ANOUSHKA\\Desktop\\srm2\\memory_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    digit_display = visual.TextStim(win=win, name='digit_display',
        text='',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from trial_logic
    ...
    response_box = visual.TextBox2(
         win, text=None, placeholder='Type the numbers you saw..', font='Georgia',
    ...
    # Run 'Begin Experiment' code from feedback_logic
    feedback_text = ""  # Initialize feedback_text as an empty string
    
    text_2 = visual.TextStim(win=win, name='text_2',
        text='',
        font='Garamond',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color=[-1.0000, 0.9608, 0.2078], colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    
    # --- Initialize components for Routine "thankyou" ---
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Thank you for participating! \nYou may not love your memory but we love to remember you :)\n\nPress enter to end your experiment',
        font='Cochin',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[digit_display],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from trial_logic
        ...
        while continueRoutine:
            ...
            # *digit_display* updates
            
            # if digit_display is starting this frame...
            if digit_display.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                digit_display.frameNStart = frameN  # exact frame index
                digit_display.tStart = t  # local t and not account for scr refresh
                digit_display.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(digit_display, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'digit_display.started')
                # update status
                digit_display.status = STARTED
                digit_display.setAutoDraw(True)
            
            # if digit_display is active this frame...
            if digit_display.status == STARTED:
                # update params
                digit_display.setText(digit_display.text, log=False)
            # Run 'Each Frame' code from trial_logic
            # Sequential presentation logic
            if presentation_type == "sequential":
        ...
        # Run 'End Routine' code from trial_logic
        # End the trial routine when trial_end_time is reached
        if t >= digit_duration:
            continueRoutine = False
        ...
        recall = data.Routine(
            name='recall',
            components=[response_box],
        )
        recall.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        response_box.reset()
        # Run 'Begin Routine' code from recall_logic
        response_box.editable = True  # Initially make the response box non-editable
        ...
        while continueRoutine:
            ...
            if response_box.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
                # keep track of start time/frame for later
            ...
            if response_box.status == STARTED:
                # update params
                pass
            # Run 'Each Frame' code from recall_logic
            # Ensure the Response Box is editable
        ...
        trials.addData('response_box.text',response_box.text)
        # Run 'End Routine' code from recall_logic
        # Gather participant response and calculate accuracy
    ...
    # Ensure the response and correct digits are strings for proper comparison
    participant_response = response_box.text.strip() if response_box.text else ""  # Handle empty response gracefully
    ...
    while continueRoutine:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['enter'], ignoreKeys=["escape"], waitRelease=False)
